...
func NewLogger(ctx context.Context, opts LoggerOptions) (*Logger, error) {
	if opts.Writers() == nil || len(opts.Writers()) <= 0 {
	...
	format := strings.ToLower(opts.LogFormat())
	if format == "auto" {
		if opts.ModeField() == types.ModeDebug {
			format = "pretty"
		} else {
			format = "json"
		}
	...
	buildBaseHandler := func(w io.Writer) slog.Handler {
		switch format {
		case "pretty", "text":
			return &DebugHandler{out: w, mut: sync.Mutex{}, level: opts.LogLevel()}
		case "json":
			fallthrough
		default:
			return slog.NewJSONHandler(w, &slog.HandlerOptions{Level: opts.LogLevel()})
		}
	}

	// Attempt OTEL handler if exporter configured
	if opts.ModeField() == types.ModeRelease {
		res, err := resource.Merge(
			resource.Default(),
			resource.NewWithAttributes(
				semconv.SchemaURL,
				semconv.ServiceName(opts.ServiceName()),
				semconv.ServiceVersion(opts.GetVersion()),
			),
		)
		if err == nil {
			var logExporter sdklog.Exporter
			switch opts.OTELExporter() {
			case "otlp-http":
				httpOpts := []otlploghttp.Option{}
				if endpoint := opts.OTELEndpoint(); endpoint != "" {
					httpOpts = append(httpOpts, otlploghttp.WithEndpoint(endpoint))
				}
				logExporter, err = otlploghttp.New(ctx, httpOpts...)
			case "otlp-grpc":
				grpcOpts := []otlploggrpc.Option{}
				if endpoint := opts.OTELEndpoint(); endpoint != "" {
					grpcOpts = append(grpcOpts, otlploggrpc.WithEndpoint(endpoint))
				}
				logExporter, err = otlploggrpc.New(ctx, grpcOpts...)
			default:
				err = fmt.Errorf("unsupported otel exporter: %s", opts.OTELExporter())
			}

			if err != nil {
				slog.Warn("failed to create OTLP log exporter, proceeding without it", "error", err)
			} else {
				...
				otelHandler := otelslog.NewHandler("otel", otelslog.WithLoggerProvider(loggerFactory))
	...
	for _, w := range opts.Writers() {
		handlers = append(handlers, buildBaseHandler(w))
	}
	mh := &MultiHandler{handlers}
	root := slog.New(mh).With(
		slog.String("service.name", opts.ServiceName()),
		slog.String("service.version", opts.GetVersion()),
		slog.String("env", string(opts.ModeField())),
	)

	return &Logger{Slogger: root, LoggerProvider: loggerFactory}, nil
}
