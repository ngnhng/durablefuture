...
func (m *Manager) runWorkflowTaskProjector(ctx context.Context) error {
	...
	cc, err := consumer.Consume(func(msg jetstream.Msg) {
		...
		case *api.WorkflowStarted:
			...
			var attrs api.WorkflowStartedAttributes
...
func (m *Manager) runActivityTaskProjector(ctx context.Context) error {
	...
	cc, err := consumer.Consume(func(msg jetstream.Msg) {
		...
		if event.EventType == api.ActivityScheduled {
			slog.Debug(fmt.Sprintf("✅ activity projector got: %v", string(msg.Data())))

			var attrs api.ActivityTaskScheduledAttributes
			if err := json.Unmarshal(event.Attributes, &attrs); err != nil {
			...
			slog.Debug(fmt.Sprintf("❌ ACTIVITY_PROJECTOR_DEBUG: Ignoring event type %d (not ActivityTaskScheduledEvent=%d)", event.EventType, api.ActivityScheduled))
		}
...
func (m *Manager) runWorkflowResultProjector(ctx context.Context) error {
	...
	cc, err := consumer.Consume(func(msg jetstream.Msg) {
		...
		if event.EventType == api.WorkflowCompleted {
			slog.Info(fmt.Sprintf("WF_RESULT_PROJECTOR: processing WorkflowCompleted: %v", event))
			var attrs api.WorkflowCompletedAttributes
			if err := json.Unmarshal(event.Attributes, &attrs); err != nil {
			...
			m.conn.Set(ctx, constant.WorkflowResultBucket, workflowID, result)
