...
func NewExecution(c *natz.Conn, conv converter2.Converter, workflowID uuid.UUID) *Execution {
	if conv == nil {
		return &Execution{
			WorkflowID: workflowID.String(),
			Conn:       c,
			converter:  converter2.NewJsonConverter(),
		}
	}
	return &Execution{
		WorkflowID: workflowID.String(),
...
func (w *Execution) Get(ctx context.Context, valuePtr any) error {
	...
	watcher, err := w.Conn.WatchKV(ctx, constant.WorkflowResultBucket, w.WorkflowID)
	if err != nil {
	...
	for update := range watcher.Updates() {
		...
		if update.Operation() == jetstream.KeyValuePut {
			...
			if valuePtr != nil && resultArray[0] != nil {
				w.converter.MustFrom(w.converter.MustTo(resultArray[0]), valuePtr)
			}
