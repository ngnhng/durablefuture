...
func (i *Impl) runWorkflowTaskProcessor(ctx context.Context) error {
	...
	workerTaskConsumer, err := js.CreateOrUpdateConsumer(ctx, constant.WorkflowTasksStream, jetstream.ConsumerConfig{
		Durable:       constant.WorkflowTaskWorkerConsumer,
		AckPolicy:     jetstream.AckExplicitPolicy,
		FilterSubject: constant.WorkflowTasksFilterSubjectPattern,
	})
	if err != nil {
		slog.Debug(fmt.Sprintf("[WORKER]: %v", err))
		...
		func() {
			defer func() {
				if r := recover(); r != nil {
					...
					if _, ok := r.(future.BlockingFutureError); !ok {
...
func (i *Impl) runActivityTaskProcessor(ctx context.Context) error {
	...
	activityTaskConsumer, err := js.CreateOrUpdateConsumer(ctx, constant.ActivityTasksStream, jetstream.ConsumerConfig{
		Durable:       constant.ActivityTaskWorkerConsumer,
		AckPolicy:     jetstream.AckExplicitPolicy,
		FilterSubject: constant.ActivityTasksFilterSubjectPattern,
	})
	if err != nil {
		return fmt.Errorf("error: %v", err)
	}
	...
	cc, err := activityTaskConsumer.Consume(
		func(msg jetstream.Msg) {
			...
			_, err = js.PublishMsg(
				ctx,
				&nats.Msg{
					Subject: fmt.Sprintf(constant.HistoryPublishSubjectPattern, task.WorkflowID),
...
func (i *Impl) loadWorkflowHistory(ctx context.Context, workflowID string) ([]types.WorkflowEvent, uint64, error) {
	...
	filter := fmt.Sprintf(constant.HistoryPublishSubjectPattern, workflowID)
...
func (i *Impl) commitEvents(ctx context.Context, workflowID string, history []types.WorkflowEvent, newEvents []types.WorkflowEvent) error {
	js, _ := i.conn.JS()
	subject := fmt.Sprintf(constant.HistoryPublishSubjectPattern, workflowID)
