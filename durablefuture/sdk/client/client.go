...
import (
	...
	converter2 "durablefuture/common/converter"
	...
	sdkconfig "durablefuture/sdk/config"
...
// Config is re-exported type alias for sdk config to keep backward compatibility if needed later.
type Config = sdkconfig.Config

// NewClient creates a client using environment loaded SDK config.
...
// NewClientWithConfig allows callers to supply their own configuration implementing natz.Config.
func NewClientWithConfig(cfg *sdkconfig.Config) (Client, error) {
...
func newClient(cfg *sdkconfig.Config) (*impl, error) {
	conn, err := natz.Connect(cfg)
	if err != nil {
		return nil, err
	}
	conv := converter2.NewJsonConverter()
	return &impl{conn: conn, converter: conv, cfg: cfg}, nil
}
...
func (c *impl) ExecuteWorkflow(ctx context.Context, workflowFn any, input ...any) (workflow.Future, error) {
	...
	reply, err := c.conn.NATS().RequestWithContext(reqCtx, constant.CommandRequestStart, commandData)
	if err != nil {
