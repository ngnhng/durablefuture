...
type (
	...
	WorkflowTaskProcessor interface {
		ReceiveTask(ctx context.Context) iter.Seq[*TaskToken]
	}
...
func NewWorker(c Client, opts *WorkerOptions) (*workerImpl, error) {
	...
	log, err := eventlog.NewNATSJetStream(c.getConn().nc)
	if err != nil {
		return nil, err
	}
	...
	return &workerImpl{
		...
		WorkflowTaskProcessor: c.getConn(),
		evtLog:                log,
...
func (w *workerImpl) runProcessingLoop(ctx context.Context) error {
	...
	for token := range w.ReceiveTask(gCtx) {
